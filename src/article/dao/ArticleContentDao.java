...
public class ArticleContentDao {
	...
	public ArticleContent selectById(Connection conn, int no) throws SQLException {
		...
		try {
			...
			if(rs.next()){
				content = convertArticleContent(rs);
			}
	...
	private ArticleContent convertArticleContent(ResultSet rs) throws SQLException {
		return new ArticleContent(rs.getInt("article_no"), rs.getString("content"));
	}
	...
	public int update(Connection conn, int no, String content) throws SQLException{
		...
		try {
			pstmt = conn.prepareStatement("update article_content set content = ? where article_no = ?");
